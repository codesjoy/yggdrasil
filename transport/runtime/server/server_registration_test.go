...
func TestRegisterRestAndRawHandlers(t *testing.T) {
	...
	require.Equal(
		t,
		restRouterInfo{Method: http.MethodGet, Path: "/api/items"},
		s.restRouterDesc[0],
	)
	require.Len(t, collector.rpcHandles, 1)
	require.Equal(
		t,
		rpcHandleCall{method: http.MethodGet, path: "/api/items"},
		collector.rpcHandles[0],
	)
	...
	require.Equal(
		t,
		rawHandleCall{method: http.MethodGet, path: "/health"},
		collector.rawHandles[1],
	)
}
