...
func newConfiguredMarshalers(cfg *MarshalerConfigSet) (marshalerSet, error) {
	return newConfiguredMarshalersWithBuilders(nil, cfg)
}

func newConfiguredMarshalersWithBuilders(
	...
	inbound, err := buildConfiguredMarshalerWithBuilders(builders, cfg.Inbound)
	if err != nil {
		return marshalerSet{}, err
	}
	outbound, err := buildConfiguredMarshalerWithBuilders(builders, cfg.Outbound)
	if err != nil {
		return marshalerSet{}, err
	}
...
func buildConfiguredMarshaler(cfg *MarshalerConfig) (marshaler.Marshaler, error) {
	return buildConfiguredMarshalerWithBuilders(nil, cfg)
}

func buildConfiguredMarshalerWithBuilders(
	builders map[string]marshaler.MarshalerBuilder,
	cfg *MarshalerConfig,
) (marshaler.Marshaler, error) {
	if cfg == nil {
		return nil, nil
	}
	if builders != nil {
		return marshaler.BuildMarshalerWithBuilders(builders, cfg.Type, cfg.Config)
	}
	return marshaler.BuildMarshalerWithConfig(cfg.Type, cfg.Config)
}
