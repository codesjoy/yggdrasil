...
func ServerProvider() remote.TransportServerProvider {
	return ServerProviderWithSettings(Settings{}, stats.NoOpHandler, nil, nil)
}
...
func ServerProviderWithSettings(
	...
	return remote.NewTransportServerProvider(
		...
		func(handle remote.MethodHandle) (remote.Server, error) {
			...
			s.securityMaterial = material
			s.securityMode = material.Mode
			s.requestAuth = material.RequestAuth
...
type server struct {
	...
	requestAuth      security.RequestAuthenticator
}
...
func (s *server) Start() error {
	...
	lis, err = buildServerListener(lis, s.securityMaterial)
	if err != nil {
		_ = lis.Close()
		return err
	}
...
func (s *server) authenticateRequest(r *http.Request) (security.AuthInfo, error) {
	...
	return s.requestAuth.AuthenticateRequest(r)
}
