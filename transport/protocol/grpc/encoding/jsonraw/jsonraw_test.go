// Copyright 2022 The codesjoy Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package jsonraw

import (
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"

	gmem "google.golang.org/grpc/mem"
)

func TestCodecMarshal(t *testing.T) {
	c := codec{}
	want := []byte(`{"message":"hello"}`)

	got, err := c.Marshal(want)
	require.NoError(t, err)
	assert.Equal(t, want, got)
}

func TestCodecMarshalRejectsNonBytes(t *testing.T) {
	c := codec{}

	_, err := c.Marshal(map[string]string{"message": "hello"})
	require.Error(t, err)
	assert.ErrorContains(t, err, "want []byte")
}

func TestCodecUnmarshal(t *testing.T) {
	c := codec{}
	data := []byte(`{"message":"hello"}`)

	var got []byte
	err := c.Unmarshal(data, &got)
	require.NoError(t, err)
	assert.Equal(t, data, got)
}

func TestCodecUnmarshalRejectsNonBytePointer(t *testing.T) {
	c := codec{}
	data := []byte(`{"message":"hello"}`)

	var got string
	err := c.Unmarshal(data, &got)
	require.Error(t, err)
	assert.ErrorContains(t, err, "want *[]byte")
}

func TestCodecMarshalV2(t *testing.T) {
	c := codec{}
	t.Run("success", func(t *testing.T) {
		want := []byte(`{"message":"hello"}`)
		bs, err := c.MarshalV2(want)
		require.NoError(t, err)
		assert.Equal(t, want, bs.Materialize())
	})

	t.Run("empty bytes returns nil", func(t *testing.T) {
		bs, err := c.MarshalV2([]byte{})
		require.NoError(t, err)
		assert.Nil(t, bs)
	})

	t.Run("non-byces error", func(t *testing.T) {
		_, err := c.MarshalV2("not bytes")
		require.Error(t, err)
		assert.ErrorContains(t, err, "want []byte")
	})
}

func TestCodecUnmarshalV2(t *testing.T) {
	c := codec{}
	t.Run("success", func(t *testing.T) {
		data := []byte(`{"message":"hello"}`)
		buf := gmem.NewBuffer(&data, nil)
		bs := gmem.BufferSlice{buf}

		var got []byte
		err := c.UnmarshalV2(bs, &got)
		require.NoError(t, err)
		assert.Equal(t, data, got)
	})

	t.Run("non-pointer error", func(t *testing.T) {
		var got string
		err := c.UnmarshalV2(nil, &got)
		require.Error(t, err)
		assert.ErrorContains(t, err, "want *[]byte")
	})
}

func TestCodecName(t *testing.T) {
	c := codec{}
	assert.Equal(t, "jsonraw", c.Name())
}
