...
// Protocol is the grpc transport protocol name.
const Protocol = "grpc"

// ClientProvider returns the built-in grpc client transport provider.
func ClientProvider() remote.TransportClientProvider {
	return ClientProviderWithSettings(Settings{}, nil)
}

// ClientProviderWithSettings returns the built-in grpc client transport provider bound to explicit settings.
func ClientProviderWithSettings(
	...
	return remote.NewTransportClientProvider(
		...
		func(
			...
			dialOpts, err := buildClientDialOptionsWithProfiles(
				cfg,
				serviceName,
				statsHandler,
				profiles,
...
// ClientConfig defines the configuration for a client.
...
func (cfg *ClientConfig) setDefault(serviceName string) {
...
func buildClientDialOptions(
	...
	serviceName string,
	statsHandler stats.Handler,
) ([]ggrpc.DialOption, error) {
	return buildClientDialOptionsWithProfiles(cfg, serviceName, statsHandler, nil)
}

func buildClientDialOptionsWithProfiles(
	cfg *ClientConfig,
	serviceName string,
	statsHandler stats.Handler,
	profiles map[string]security.Profile,
) ([]ggrpc.DialOption, error) {
	creds, err := buildTransportCredentialsWithProfiles(
		profiles,
		cfg.Transport.SecurityProfile,
		serviceName,
		true,
		cfg.Transport.Authority,
	)
	if err != nil {
		return nil, err
	}
