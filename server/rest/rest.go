...
type ServeMux struct {
	...
	middlewareMap     map[string]middleware.Provider
}
...
func WithMiddlewareProviders(providers map[string]middleware.Provider) Option {
	return func(s *ServeMux) {
...
func NewServer(cfg *Config, opts ...Option) (Server, error) {
	...
	if s.middlewareMap != nil {
		r.Use(middleware.BuildWithProviders(s.middlewareMap, allMiddlewares...)...)
	} else {
		r.Use(middleware.Build(allMiddlewares...)...)
	}
	...
	rpcRouter := r.Group(func(r chi.Router) {
		...
		if s.middlewareMap != nil {
			r.Use(middleware.BuildWithProviders(s.middlewareMap, rpcMiddlewares...)...)
		} else {
			r.Use(middleware.Build(rpcMiddlewares...)...)
		}
	})
	...
	webRouter := r.Group(func(r chi.Router) {
		if s.middlewareMap != nil {
			r.Use(middleware.BuildWithProviders(s.middlewareMap, webMiddlewares...)...)
		} else {
			r.Use(middleware.Build(webMiddlewares...)...)
		}
	})
