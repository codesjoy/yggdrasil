...
import (
	...
	"github.com/codesjoy/yggdrasil/pkg/config/internal"
	"github.com/codesjoy/yggdrasil/pkg/config/source"

	"github.com/codesjoy/yggdrasil/pkg/utils/xgo"
	"github.com/codesjoy/yggdrasil/pkg/utils/xmap"
)
...
func (c *config) watchSource(source source.Source) {
	...
	if err != nil {
		slog.Error("fault to watch config source", slog.Any("error", err))
		return
	}

	for change := range changeCh {
		if err := c.loadSourceData(change); err != nil {
			slog.Error("fault to load source data", slog.Any("error", err))
			continue
		}
		if err := c.apply(); err != nil {
			slog.Error("fault to apply source data", slog.Any("error", err))
		}
	}
}

func (c *config) loadSourceData(sourceData source.Data) error {
...
func (c *config) loadSource(sources ...source.Source) error {
	...
	for _, item := range sources {
		...
		if item.Changeable() {
			xgo.Go(func() { c.watchSource(item) })
		}
	}
	return nil
}
...
func (c *config) notify(changes map[string]WatchEventType, version uint64, val Values) {
	...
	changedWatchPrefixMap := map[string]WatchEventType{}
...
func (c *config) hasPrefix(key, watchPrefix string) bool {
	return internal.HasPrefix(key, watchPrefix, c.keyDelimiter)
}
