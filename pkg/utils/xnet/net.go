...
// Package xnet provides net utils
package xnet
...
// GetHostAndPortByAddr returns host and port from net.Addr
...
// GetPort returns a free port
func GetPort() int {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	l, _ := (&net.ListenConfig{}).Listen(ctx, "tcp", ":0") // listen on localhost
	defer func() {
		_ = l.Close()
	}()
...
// GetAddrAndPort returns a free port
