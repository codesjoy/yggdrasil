...
func ClientProviderWithSettings(
	...
	return remote.NewTransportClientProvider(
		...
		func(
			...
			dialOpts, err := buildClientDialOptionsWithBuilders(cfg, serviceName, statsHandler, builders)
			if err != nil {
				return nil, err
			}
...
func buildClientDialOptions(cfg *Config, serviceName string, statsHandler stats.Handler) ([]ggrpc.DialOption, error) {
	return buildClientDialOptionsWithBuilders(cfg, serviceName, statsHandler, nil)
}

func buildClientDialOptionsWithBuilders(
	cfg *Config,
	serviceName string,
	statsHandler stats.Handler,
	builders map[string]credentials.Builder,
) ([]ggrpc.DialOption, error) {
	creds, err := buildTransportCredentialsWithBuilders(builders, cfg.Transport.CredsProto, serviceName, true)
	if err != nil {
		return nil, err
	}
