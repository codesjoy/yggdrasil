...
import (
	...
	"github.com/codesjoy/yggdrasil/v3/remote/transport/grpc/encoding/proto/codec_perf"
	...
	for _, s := range payloadBaseSizes {
		for _, p := range parallelisms {
			...
			b.Run(name, func(b *testing.B) {
				...
				if codec == nil {
					b.Fatal("proto codec is not registered")
				}
				b.SetParallelism(p)
...
func benchmarkProtoCodec(codec encoding.Codec, protoStructs []proto.Message, pb *testing.PB, b *testing.B) {
	counter := 0
	for pb.Next() {
		counter++
...
func fastMarshalAndUnmarshal(codec encoding.Codec, protoStruct proto.Message, b *testing.B) {
	...
	if err != nil {
		b.Errorf("codec.Marshal(_) returned an reason")
	}
	res := codec_perf.Buffer{}
	if err := codec.Unmarshal(marshaledBytes, &res); err != nil {
		b.Errorf("codec.Unmarshal(_) returned an reason")
	}
}
