...
func ServerProviderWithSettings(
	...
	return remote.NewTransportServerProvider(Protocol, func(handle remote.MethodHandle) (remote.Server, error) {
		...
		if err := opts.SetDefaultWithBuilders(builders); err != nil {
			return nil, err
		}
...
func (opts *serverOptions) SetDefault() error {
	return opts.SetDefaultWithBuilders(nil)
}

func (opts *serverOptions) SetDefaultWithBuilders(builders map[string]credentials.Builder) error {
	if opts.Network == "" {
		opts.Network = "tcp"
	}
	...
	if opts.CredsProto != "" {
		creds, err := buildTransportCredentialsWithBuilders(builders, opts.CredsProto, "", false)
		if err != nil {
			return err
		}
		opts.creds = creds
