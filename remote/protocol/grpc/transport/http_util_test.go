...
func (s) TestDecodeEncodeGrpcMessage(t *testing.T) {
	testCases := []struct {
		...
		{
			string([]byte{0xff}) + "Hello" + string([]byte{0xfe}) + "世界" + string([]byte{0xfd}),
			"�Hello�世界�",
		},
	}
	for _, tC := range testCases {
		...
		if got != tC.want {
			t.Errorf(
				"decodeGrpcMessage(encodeGrpcMessage(%q)) = %q, want %q",
				tC.orig,
				got,
				tC.want,
			)
...
func (s) TestDecodeMetadataHeader(t *testing.T) {
	for _, test := range []struct {
		...
		if !reflect.DeepEqual(v, test.vout) || !reflect.DeepEqual(err, test.err) {
			t.Fatalf(
				"decodeMetadataHeader(%q, %q) = %q, %v, want %q, %v",
				test.kin,
				test.vin,
				v,
				err,
...
func (s) TestParseDialTarget(t *testing.T) {
	for _, test := range []struct {
		...
		if gotNet != test.wantNet || gotAddr != test.wantAddr {
			t.Errorf(
				"parseDialTarget(%q) = %s, %s want %s, %s",
				test.target,
				gotNet,
				gotAddr,
				test.wantNet,
