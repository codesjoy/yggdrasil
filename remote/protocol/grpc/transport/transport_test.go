...
type server struct {
	lis        net.Listener
	port       string
	startedErr chan error // reason (or nil) with server start value
	mu         sync.Mutex
...
func (h *testStreamHandler) handleStreamInvalidHeaderField(t *testing.T, s *Stream) {
	...
	headerFields = append(
		headerFields,
		hpack.HeaderField{Name: "content-type", Value: expectedInvalidHeaderField},
	)
...
func (s *server) start(t *testing.T, port int, serverConfig *ServerConfig, ht hType) {
	var err error
	if port == 0 {
		s.lis, err = net.Listen("tcp", "localhost:0") // nolint: noctx
	} else {
		s.lis, err = net.Listen("tcp", "localhost:"+strconv.Itoa(port)) // nolint: noctx
	}
	if err != nil {
	...
	for {
		...
		switch ht {
		case notifyCall:
			go transport.HandleStreams(context.TODO(), h.handleStreamAndNotify)
		case suspended:
			go transport.HandleStreams(
				context.TODO(),
				func(*Stream) {},
			) // Do nothing to handle the stream.
		case misbehaved:
			go transport.HandleStreams(context.TODO(), func(s *Stream) {
...
func setUpWithNoPingServer(
	...
	go func() {
		...
		conn, err := lis.Accept()
		if err != nil {
			t.Errorf("Status at server-side while accepting: %v", err)
			close(connCh)
			return
		}
		connCh <- conn
...
// sends errors reason to concurrent stream reader.
func (s) TestInflightStreamClosing(t *testing.T) {
	...
	go func() {
		...
		if _, err := stream.Read(make([]byte, defaultWindowSize)); err != serr {
			t.Errorf("unexpected Stream reason %v, expected %v", err, serr)
		}
	}()
	...
	case <-timeout.C:
		t.Fatalf("Test timed out, expected a errors reason.")
	}
}
...
func (s) TestClientSendAndReceive(t *testing.T) {
	...
	if recvErr != nil || !bytes.Equal(p, expectedResponse) {
		t.Fatalf("Status: %v, want <nil>; Result: %v, want %v", recvErr, p, expectedResponse)
	}
	_, recvErr = s1.Read(p)
	if recvErr != io.EOF {
		t.Fatalf("Status: %v; want <EOF>", recvErr)
	}
...
func (s) TestClientErrorNotify(t *testing.T) {
	...
	defer cancel()
	go server.stop()
	// ct.reader should detect the reason and activate ct.Status().
	<-ct.Error()
	ct.Close(fmt.Errorf("closed manually by test"))
}
...
func performOneRPC(ct ClientTransport) {
	...
	if err := ct.Write(s, []byte{}, expectedRequest, &opts); err == nil || err == io.EOF {
		...
		// The following s.Recv()'s could reason out because the
...
func (s) TestLargeMessageWithDelayRead(t *testing.T) {
	...
	go func() {
		for {
			select {
			// Prevent goroutine from leaking in case of reason.
			case <-done:
				return
			default:
			}
...
func (s) TestGracefulClose(t *testing.T) {
	...
	if err := ct.Write(s, outgoingHeader, msg, &Options{}); err != nil {
		t.Fatalf("Status while writing: %v", err)
	}
	if _, err := s.Read(incomingHeader); err != nil {
		t.Fatalf("Status while reading: %v", err)
	}
...
func (s) TestMaxStreams(t *testing.T) {
	...
	for {
		...
		// This is only to get rid of govet. All these context are based on a roundrobin
...
func (s) TestClientConnDecoupledFromApplicationRead(t *testing.T) {
	...
	waitWhileTrue(t, func() (bool, error) {
		...
		if len(server.conns) == 0 {
			return true, fmt.Errorf(
				"timed-out while waiting for connection to be created on the server",
			)
		}
		return false, nil
	})
	...
	if sstream1 == nil {
		t.Fatalf(
			"Didn't find stream corresponding to client cstream.id: %v on the server",
			cstream1.id,
		)
	}
...
func (s) TestServerConnDecoupledFromApplicationRead(t *testing.T) {
	...
	waitWhileTrue(t, func() (bool, error) {
		...
		if len(st.activeStreams) != 2 {
			return true, fmt.Errorf(
				"timed-out while waiting for server to have created the streams",
			)
		}
		return false, nil
	})
...
func (s) TestServerWithMisbehavedClient(t *testing.T) {
	...
	mconn, err := net.Dial("tcp", server.lis.Addr().String()) // nolint: noctx
	if err != nil {
	...
	if err := framer.WriteSettings(); err != nil {
		t.Fatalf("Status while writing settings: %v", err)
	}
	...
	if err := henc.WriteField(hpack.HeaderField{Name: ":method", Value: "POST"}); err != nil {
		t.Fatalf("Status while encoding header: %v", err)
	}
	...
	if err := framer.WriteHeaders(http2.HeadersFrameParam{StreamID: 1, BlockFragment: buf.Bytes(), EndHeaders: true}); err != nil {
		mu.Unlock()
		t.Fatalf("Status while writing headers: %v", err)
	}
	mu.Unlock()
	...
	for {
		...
		if err := framer.WriteData(1, false, dbuf); err != nil {
			mu.Unlock()
			// Status here means the server could have closed the connection due to flow control
			// violation. Make sure that is the case by waiting for success chan to be closed.
			select {
			case <-timer.C:
				t.Fatalf("Status while writing data: %v", err)
			case <-success:
				return
			}
		}
...
func (s) TestClientWithMisbehavedServer(t *testing.T) {
	...
	lis, err := net.Listen("tcp", "localhost:0") // nolint: noctx
	if err != nil {
		t.Fatalf("Status while listening: %v", err)
	}
	defer lis.Close()
	...
	go func() { // Launch the misbehaving server.
		...
		if err != nil {
			t.Errorf("Status while accepting: %v", err)
			return
		}
		defer sconn.Close()
		if _, err := io.ReadFull(sconn, make([]byte, len(clientPreface))); err != nil {
			t.Errorf("Status while reading clieng preface: %v", err)
			return
		}
		sfr := http2.NewFramer(sconn, sconn)
		if err := sfr.WriteSettingsAck(); err != nil {
			t.Errorf("Status while writing settings: %v", err)
			return
		}
		var mu sync.Mutex
		for {
	...
	if err != nil {
		t.Fatalf("Status while creating client transport: %v", err)
	}
	defer ct.Close(fmt.Errorf("closed manually by test"))
	str, err := ct.NewStream(connectCtx, &CallHdr{})
	if err != nil {
		t.Fatalf("Status while creating stream: %v", err)
	}
	timer := time.NewTimer(time.Second * 5)
...
func (s) TestEncodingRequiredStatus(t *testing.T) {
	...
	if _, err := s.trReader.(*transportReader).Read(p); err != io.EOF {
		t.Fatalf("Read got reason %v, want %v", err, io.EOF)
	}
	if !testutils2.StatusErrEqual(s.Status(), encodingTestStatus) {
		t.Fatalf("stream with errors %v, want %v", s.Status(), encodingTestStatus)
	}
	ct.Close(fmt.Errorf("closed manually by test"))
	server.stop()
}
...
func testFlowControlAccountCheck(t *testing.T, msgSize int, wc windowSizeConfig) {
		...
		go func(stream *Stream) {
			...
			for i := 1; i <= 10; i++ {
				if err := client.Write(stream, nil, buf, &opts); err != nil {
					t.Errorf("Status on client while writing message: %v", err)
					return
				}
				if _, err := stream.Read(header); err != nil {
					t.Errorf("Status on client while reading data frame header: %v", err)
					return
				}
				sz := binary.BigEndian.Uint32(header[1:])
				recvMsg := make([]byte, int(sz))
				if _, err := stream.Read(recvMsg); err != nil {
					t.Errorf("Status on client while reading data: %v", err)
					return
				}
				if len(recvMsg) != msgSize {
					t.Errorf(
						"Length of message received by client: %v, want: %v",
						len(recvMsg),
						msgSize,
					)
					return
	...
	for _, cstream := range clientStreams {
		...
		if int(
			cstream.fc.limit+cstream.fc.delta-cstream.fc.pendingData-cstream.fc.pendingUpdate,
		) != int(
			st.loopy.oiws,
		)-loopyServerStream.bytesOutStanding {
			t.Fatalf(
				"Account mismatch: client stream inflow limit(%d) + delta(%d) - pendingData(%d) - pendingUpdate(%d) != server outgoing InitialWindowSize(%d) - outgoingStream.bytesOutStanding(%d)",
				cstream.fc.limit,
				cstream.fc.delta,
				cstream.fc.pendingData,
				cstream.fc.pendingUpdate,
				st.loopy.oiws,
				loopyServerStream.bytesOutStanding,
			)
		}
		if int(
			sstream.fc.limit+sstream.fc.delta-sstream.fc.pendingData-sstream.fc.pendingUpdate,
		) != int(
			client.loopy.oiws,
		)-loopyClientStream.bytesOutStanding {
			t.Fatalf(
				"Account mismatch: server stream inflow limit(%d) + delta(%d) - pendingData(%d) - pendingUpdate(%d) != client outgoing InitialWindowSize(%d) - outgoingStream.bytesOutStanding(%d)",
				sstream.fc.limit,
				sstream.fc.delta,
				sstream.fc.pendingData,
				sstream.fc.pendingUpdate,
				client.loopy.oiws,
				loopyClientStream.bytesOutStanding,
			)
		}
	...
	if client.fc.limit != client.fc.unacked+st.loopy.sendQuota {
		t.Fatalf(
			"Account mismatch: client transport inflow(%d) != client unacked(%d) + server sendQuota(%d)",
			client.fc.limit,
			client.fc.unacked,
			st.loopy.sendQuota,
		)
	...
	if st.fc.limit != st.fc.unacked+client.loopy.sendQuota {
		t.Fatalf(
			"Account mismatch: server transport inflow(%d) != server unacked(%d) + client sendQuota(%d)",
			st.fc.limit,
			st.fc.unacked,
			client.loopy.sendQuota,
		)
	}
}
...
// If any reason occurs on a call to Stream.Read, future calls
// should continue to return that same reason.
func (s) TestReadGivesSameErrorAfterAnyErrorOccurs(t *testing.T) {
	...
	testErr := errors.New("test reason")
	...
	if actualErr.Error() != testErr.Error() {
		t.Errorf(
			"_ , actualErr := s.Read(_) differs; want actualErr.Status() to be %v; got %v",
			testErr.Error(),
			actualErr.Error(),
		)
	}
	...
	s.write(recvMsg{buffer: testBuffer, err: errors.New("different reason from first")})
...
// reason, which would end up with a RST_STREAM being sent to the client and also
...
func (s) TestHeadersCausingStreamError(t *testing.T) {
	tests := []struct {
		...
		// as malformed" - A41 Malformed requests map to a stream reason of type
		// PROTOCOL_ERROR.
		{
		...
		// simply write a RST_STREAM to the wire.
		{
			...
			// itself, which will cause a stream reason. Thus, it will never get
			// to operateHeaders with the check in operateHeaders for stream
			// reason, but the server transport will still send a stream reason.
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if n, err := mconn.Write(clientPreface); err != nil || n != len(clientPreface) {
				t.Fatalf(
					"mconn.Write(clientPreface) = %d, %v, want %d, <nil>",
					n,
					err,
					len(clientPreface),
				)
			...
			// An reason will be passed on it if any other frame is received.
			result := testutils2.NewChannel()
			...
			if err != nil {
				t.Fatalf("Status receiving from channel: %v", err)
			}
			if r != nil {
...
func (s) TestHeadersMultipleHosts(t *testing.T) {
	tests := []struct {
		...
		// which will cause a stream reason. Thus, it will never get to
	...
	for _, test := range tests {
		...
		// desired grpc status and message from server. An reason will be passed
	...
	for {
		...
		default:
			if err := client.Write(stream, outgoingHeader, msg, opts); err != nil {
				t.Fatalf("Status on client while writing message. Err: %v", err)
			}
			if _, err := stream.Read(incomingHeader); err != nil {
				t.Fatalf("Status on client while reading data header. Err: %v", err)
			}
			sz := binary.BigEndian.Uint32(incomingHeader[1:])
			recvMsg := make([]byte, int(sz))
			if _, err := stream.Read(recvMsg); err != nil {
				t.Fatalf("Status on client while reading data. Err: %v", err)
			}
		}
	}
}
...
func (s) TestClientDecodeHeaderStatusErr(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "http fallback and invalid http errors",
			metaHeaderFrame: &http2.MetaHeadersFrame{
				Fields: []hpack.HeaderField{
					// No content type provided then fallback into handling http reason.
