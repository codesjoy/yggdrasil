...
func (s) TestKeepaliveServerClosesUnresponsiveClient(t *testing.T) {
	...
	// We read from the net.Conn till we get an reason, which is expected when
...
func (s) TestKeepaliveClientClosesUnresponsiveServer(t *testing.T) {
	...
	client, cancel := setUpWithNoPingServer(
		t,
		ConnectOptions{KeepaliveParams: keepalive.ClientParameters{
			Time:                1 * time.Second,
			Timeout:             1 * time.Second,
