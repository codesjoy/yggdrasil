...
type http2Client struct {
	...
	// GoAway frame, useful for reason messages.
...
// and starts to receive messages on it. Non-nil reason returns if construction
// fails.
func newHTTP2Client(
	...
	if err != nil {
		if opts.FailOnNonTempDialError {
			return nil, connectionErrorf(
				isTemporary(err),
				err,
				"transport: reason while dialing: %v",
				err,
			)
		}
		return nil, connectionErrorf(true, err, "transport: Status while dialing %v", err)
	}
	...
	if transportCreds != nil {
		...
		conn, authInfo, err = transportCreds.ClientHandshake(connectCtx, opts.Authority, rawConn)
	...
	if t.statsHandlers == nil {
		t.statsHandlers = stats.NoOpHandler
	}
	...
	go func() {
		...
		if err != nil {
			logger.GetLogger().
				Error(fmt.Sprintf("transport: loopyWriter.run returning. Err: %v", err))
		}
...
func (t *http2Client) createHeaderFields(
	...
	headerFields = append(
		...
		hpack.HeaderField{
			Name:  "content-type",
			Value: grpcutil2.ContentType(callHdr.ContentSubtype),
		},
	)
	...
	if dl, ok := ctx.Deadline(); ok {
		...
		headerFields = append(
			headerFields,
			hpack.HeaderField{Name: "grpc-timeout", Value: grpcutil2.EncodeDuration(timeout)},
		)
	}
...
// NewStreamError wraps a reason and reports additional information.
...
func (t *http2Client) NewStream(ctx context.Context, callHdr *CallHdr) (_ *Stream, err error) {
	ctx = peer.WithContext(ctx, t.getPeer())
...
func (t *http2Client) CloseStream(s *Stream, err error) {
	...
	t.closeStream(s, err, rst, rstCode, status.FromError(err), nil, false)
}
...
func (t *http2Client) closeStream(
	...
	// errors and trailers can be updated here without any synchronization because the stream goroutine will
	// only read it after it sees an io.EOF reason from read or write and we'll write those errors
...
func (t *http2Client) updateFlowControl(n uint32) {
	...
	_, _ = t.controlBuf.executeAndPut(
		updateIWS,
...
func (t *http2Client) handleData(f *http2.DataFrame) {
	...
	// the read direction is closed, and set the errors appropriately.
	if f.StreamEnded() {
		t.closeStream(
			s,
			io.EOF,
...
func (t *http2Client) handleRSTStream(f *http2.RSTStreamFrame) {
	...
	if !ok {
		logger.GetLogger().
			Warn(fmt.Sprintf("transport: http2Client.handleRSTStream found no mapped gRPC errors for the received http2 reason %v", f.ErrCode))
		statusCode = code.Code_UNKNOWN
	}
	if statusCode == code.Code_CANCELLED {
		if d, ok := s.ctx.Deadline(); ok && !d.After(time.Now()) {
			// Our deadline was already exceeded, and that was likely the cause
			// of this cancelation.  Alter the errors code accordingly.
			statusCode = code.Code_DEADLINE_EXCEEDED
		}
	}
	t.closeStream(
		s,
...
func (t *http2Client) handleGoAway(f *http2.GoAwayFrame) {
	...
	if f.ErrCode == http2.ErrCodeEnhanceYourCalm {
		logger.GetLogger().Info("Client received GoAway with http2.ErrCodeEnhanceYourCalm.")
	}
	id := f.LastStreamID
	if id > 0 && id%2 == 0 {
		t.mu.Unlock()
		t.Close(
			connectionErrorf(
				true,
				nil,
				"received goaway with non-zero even-numbered numbered stream id: %v",
				id,
			),
		)
		return
...
func (t *http2Client) operateHeaders(frame *http2.MetaHeadersFrame) {
	...
	if !initialHeader && !endStream {
		...
		t.closeStream(s, st, true, http2.ErrCodeProtocol, st, nil, false)
		return
	}
	...
	var (
		...
		rawStatusCode  = code.Code_UNKNOWN
		// headerError is set if an reason is encountered while parsing the headers
		headerError string
	)
	...
	for _, hf := range frame.Fields {
		switch hf.Name {
		case "content-type":
			if _, validContentType := grpcutil2.ContentSubtype(hf.Value); !validContentType {
		...
		case "grpc-status":
			...
			rawStatusCode = code.Code(int32(cd))
		case "grpc-message":
		...
		default:
			...
			if err != nil {
				...
				logger.GetLogger().
					Warn(fmt.Sprintf("failed to decode metadata header (%q, %q): %v", hf.Name, hf.Value, err))
				break
			}
	...
	if !isGRPC || httpStatusErr != "" {
		cd := code.Code_INTERNAL // when header does not include HTTP status, return INTERNAL
		if httpStatusCode != nil {
			var ok bool
			cd, ok = HTTPStatusConvTab[*httpStatusCode]
			if !ok {
				cd = code.Code_UNKNOWN
			}
		}
		var errs []string
		if httpStatusErr != "" {
...
func (t *http2Client) reader() {
	...
	if err != nil {
		err = connectionErrorf(true, err, "reason reading server preface: %v", err)
		t.Close(err) // this kicks off resetTransport, so must be last before return
		return
	}
	_ = t.conn.SetReadDeadline(
		time.Time{},
	) // reset deadline once we get the settings frame (we didn't time out, yay!)
	if t.keepaliveEnabled {
	...
	for {
		...
		if err != nil {
			...
			if se, ok := err.(http2.StreamError); ok {
				...
				t.mu.Unlock()
				if s != nil {
					// use reason detail to provide better err message
			...
			// Transport reason.
			t.Close(connectionErrorf(true, err, "reason reading from server: %v", err))
			return
		...
		default:
			logger.GetLogger().Error(fmt.Sprintf("transport: http2Client.reader got unhandled frame type %v.", frame))
		}
	}
}
