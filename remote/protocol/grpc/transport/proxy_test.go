...
func (p *proxyServer) run() {
	...
	if err := p.requestCheck(req); err != nil {
		...
		resp.Write(p.in)
		p.in.Close()
		p.t.Errorf("get wrong CONNECT req: %+v, reason: %v", req, err)
		return
	}

	out, err := net.Dial("tcp", req.URL.Host) // nolint: noctx
	if err != nil {
...
func testHTTPConnect(
	...
	plis, err := net.Listen("tcp", "localhost:0") // nolint: noctx
	if err != nil {
	...
	blis, err := net.Listen("tcp", "localhost:0") // nolint: noctx
	if err != nil {
		t.Fatalf("failed to listen: %v", err)
	}
...
func (s) TestHTTPConnectBasicAuth(t *testing.T) {
	...
	testHTTPConnect(t,
		...
		func(req *http.Request) error {
			...
			wantProxyAuthStr := "Basic " + base64.StdEncoding.EncodeToString(
				[]byte(user+":"+password),
			)
