...
func TestSimpleParsing(t *testing.T) {
	...
	for _, test := range []struct {
		...
		p := &parser{r: buf, recvBufferPool: getShareBufferPool()}
		pt, b, err := p.recvMsg(math.MaxInt32)
		if !errors.Is(err, test.err) || !bytes.Equal(b, test.b) || pt != test.pt {
			t.Fatalf(
				"parser{%v}.recvMsg(_) = %v, %v, %v\nwant %v, %v, %v",
				test.p,
				pt,
				b,
				err,
...
func TestMultipleParsing(t *testing.T) {
	...
	pr := &parser{r: b, recvBufferPool: getShareBufferPool()}
	...
	for i, want := range wantRecvs {
		pt, data, err := pr.recvMsg(math.MaxInt32)
...
func TestToRPCErr(t *testing.T) {
	for _, test := range []struct {
		...
		if _, ok := status.CoverError(err); !ok {
			t.Errorf("toRPCErr{%v} returned type %T, want %T", test.errIn, err, test.errOut)
		}
...
func bmEncode(b *testing.B, mSize int) {
	...
	msg := &codec_perf.Buffer{Body: make([]byte, mSize)}
