# ConfigMap 配置源示例

本示例演示如何使用 contrib/k8s 的 ConfigMap 配置源功能，从 Kubernetes ConfigMap 读取配置并支持配置热更新。

## 你会得到什么

- 从 Kubernetes ConfigMap 加载配置
- 自动解析 YAML/JSON/TOML 格式（根据文件扩展名自动识别）
- 支持配置热更新，监听 ConfigMap 变化
- 按优先级管理多个配置源
- 实时演示配置变更通知

## 前置条件

1. 有一个可访问的 Kubernetes 集群
2. 已在集群中部署了包含配置的 ConfigMap
3. 本地安装了 Go 1.19+
4. kubectl 已配置好集群访问权限

### 快速启动本地 Kubernetes

如果你没有 Kubernetes 集群，可以使用以下工具之一快速启动：

**使用 Minikube：**
```bash
minikube start
```

**使用 Kind：**
```bash
kind create cluster
```

**使用 Docker Desktop：**
```bash
# Docker Desktop 内置了 Kubernetes，在设置中启用即可
```

验证 Kubernetes 集群是否正常运行：
```bash
kubectl cluster-info
kubectl get nodes
```

## 启动方式

### 1. 创建 ConfigMap

首先在 Kubernetes 中创建包含配置的 ConfigMap：

```bash
kubectl create configmap example-config --from-literal=config.yaml='message: "Hello from ConfigMap!"' -n default
```

或者使用 YAML 文件创建：

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: example-config
  namespace: default
data:
  config.yaml: |
    message: "Hello from ConfigMap!"
    database:
      host: localhost
      port: 5432
    cache:
      enabled: true
      ttl: 3600
```

创建 ConfigMap：
```bash
kubectl apply -f configmap.yaml
```

### 2. 验证 ConfigMap

查看 ConfigMap 内容：
```bash
kubectl get configmap example-config -n default -o yaml
```

### 3. 修改配置（可选）

如果 Kubernetes 集群地址不是默认的，请修改 [main.go](main.go) 中的 Namespace 和 Name 配置：

```go
src, err := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Namespace: "your-namespace",
    Name:      "your-config",
    Key:       "config.yaml",
    Watch:     true,
    Priority:  source.PriorityRemote,
})
```

或者通过环境变量修改（如果使用环境变量读取配置）：

```bash
export KUBERNETES_NAMESPACE=your-namespace
```

### 4. 运行示例

```bash
cd example/config-source
go run main.go
```

程序将从 ConfigMap 中读取配置并监听变化。

## 预期输出

```
watching config changes, press Ctrl+C to exit...
config changed: type=1, version=1
message: Hello from ConfigMap!
```

## 测试配置更新

### 方式 1：使用 kubectl 更新

```bash
kubectl patch configmap example-config -n default --type='json' -p='{"data":{"config.yaml":"message: \"Updated message!\nversion: 2.0"}}'
```

或者直接替换：
```bash
kubectl create configmap example-config --from-literal=config.yaml='message: "Updated message!"' --dry-run=client -o yaml | kubectl apply -f -
```

### 方式 2：使用 YAML 文件更新

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: example-config
  namespace: default
data:
  config.yaml: |
    message: "Updated from YAML file!"
    version: "2.0"
    settings:
      debug: true
      maxConnections: 100
```

```bash
kubectl apply -f updated-configmap.yaml
```

观察程序输出，应该能看到配置变化的通知：
```
config changed: type=2, version=2
message: Updated from YAML file!
```

### 方式 3：使用 kubectl edit

```bash
kubectl edit configmap example-config -n default
```

编辑后保存，程序会自动检测到变化。

## 配置说明

### ConfigSourceConfig 参数

| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `Namespace` | string | Kubernetes 命名空间 | 从环境变量 `KUBERNETES_NAMESPACE` 或 `default` |
| `Name` | string | ConfigMap/Secret 名称 | 必填 |
| `Key` | string | ConfigMap 中的 key | 必填 |
| `MergeAllKey` | bool | 是否合并所有 key | `false` |
| `Format` | Parser | 配置解析器 | 根据文件扩展名自动推断 |
| `Priority` | Priority | 配置优先级 | `source.PriorityRemote` |
| `Watch` | bool | 是否监听配置变更 | `false` |
| `Kubeconfig` | string | kubeconfig 文件路径 | 空（使用 in-cluster config） |

### 工作模式

**单 Key 模式（默认）：**
```go
src, err := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Namespace: "default",
    Name:      "my-config",
    Key:       "config.yaml",
    Watch:     true,
})
```
- 只读取 ConfigMap 中的 `config.yaml` key
- 根据 `.yaml` 扩展名自动使用 YAML 解析器
- 支持热更新

**多 Key 合并模式：**
```go
src, err := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Namespace: "default",
    Name:      "my-config",
    MergeAllKey: true,
    Watch:     true,
})
```
- 读取 ConfigMap 中的所有 key
- 将所有 key 合并为一个 map
- 不进行格式解析，直接作为 map 注入

### 配置解析

根据 Key 的扩展名自动选择解析器：

| 扩展名 | 解析器 | 示例 |
|--------|--------|------|
| `.json` | JSON | `config.json` |
| `.yaml`, `.yml` | YAML | `config.yaml` |
| `.toml` | TOML | `config.toml` |
| 其他 | YAML（默认） | `config` |

### 代码结构说明

```go
// 1. 创建 ConfigMap 配置源
src, err := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Namespace: "default",
    Name:      "example-config",
    Key:       "config.yaml",
    Watch:     true,
    Priority:  source.PriorityRemote,
})
if err != nil {
    panic(err)
}

// 2. 加载配置源到框架
if err := config.LoadSource(src); err != nil {
    panic(err)
}

// 3. 添加配置变更监听器
if err := config.AddWatcher("example", func(ev config.WatchEvent) {
    fmt.Printf("config changed: type=%v, version=%d\n", ev.Type(), ev.Version())
    
    if ev.Type() == config.WatchEventUpd || ev.Type() == config.WatchEventAdd {
        var cfg struct {
            Message string `mapstructure:"message"`
        }
        if err := ev.Value().Scan(&cfg); err != nil {
            fmt.Printf("failed to scan config: %v\n", err)
            return
        }
        fmt.Printf("message: %s\n", cfg.Message)
    }
}); err != nil {
    panic(err)
}

// 4. 等待信号
sig := make(chan os.Signal, 1)
signal.Notify(sig, syscall.SIGINT, syscall.SIGTERM)
<-sig
```

## 高级用法

### 多配置源优先级

```go
// 1. 加载本地配置（低优先级）
localSrc, _ := source.NewFileSource("config.local.yaml", source.PriorityLocal)
config.LoadSource(localSrc)

// 2. 加载 ConfigMap 配置（高优先级，覆盖本地配置）
configMapSrc, _ := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Namespace: "default",
    Name:      "my-config",
    Key:       "config.yaml",
    Priority:  source.PriorityRemote,
})
config.LoadSource(configMapSrc)

// ConfigMap 的配置会覆盖本地配置的同名字段
```

### 嵌套配置

ConfigMap 中的 YAML 配置支持嵌套结构：

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: app-config
data:
  config.yaml: |
    server:
      host: "0.0.0.0"
      port: 8080
      timeout: 30s
    database:
      host: "db.example.com"
      port: 5432
      name: "mydb"
      pool:
        min: 5
        max: 20
    cache:
      type: "redis"
      host: "cache.example.com"
      port: 6379
      ttl: 300
```

在代码中读取：
```go
var cfg struct {
    Server   struct {
        Host    string `mapstructure:"host"`
        Port    int    `mapstructure:"port"`
        Timeout string `mapstructure:"timeout"`
    } `mapstructure:"server"`
    Database struct {
        Host  string `mapstructure:"host"`
        Port  int    `mapstructure:"port"`
        Name  string `mapstructure:"name"`
        Pool  struct {
            Min int `mapstructure:"min"`
            Max int `mapstructure:"max"`
        } `mapstructure:"pool"`
    } `mapstructure:"database"`
    Cache struct {
        Type string `mapstructure:"type"`
        Host string `mapstructure:"host"`
        Port int    `mapstructure:"port"`
        TTL  int    `mapstructure:"ttl"`
    } `mapstructure:"cache"`
}
config.Get("app").Scan(&cfg)
```

### 数组配置

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: array-config
data:
  config.yaml: |
    servers:
      - host: "10.0.0.1"
        port: 8080
      - host: "10.0.0.2"
        port: 8080
      - host: "10.0.0.3"
        port: 8080
```

在代码中读取：
```go
var cfg struct {
    Servers []struct {
        Host string `mapstructure:"host"`
        Port int    `mapstructure:"port"`
    } `mapstructure:"servers"`
}
config.Get("app").Scan(&cfg)
for _, server := range cfg.Servers {
    fmt.Printf("server: %s:%d\n", server.Host, server.Port)
}
```

### 多环境配置

使用不同的 ConfigMap 管理不同环境的配置：

```go
// 根据环境变量选择不同的 ConfigMap
env := os.Getenv("APP_ENV")
if env == "" {
    env = "dev"
}

configMapName := fmt.Sprintf("app-config-%s", env)

src, err := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Namespace: "default",
    Name:      configMapName,
    Key:       "config.yaml",
    Watch:     true,
})
```

## RBAC 权限

程序需要以下 RBAC 权限才能访问 ConfigMap：

```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
  namespace: default
  name: configmap-reader
rules:
- apiGroups: [""]
  resources: ["configmaps"]
  verbs: ["get", "list", "watch"]
---
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding
metadata:
  name: configmap-reader
  namespace: default
subjects:
- kind: ServiceAccount
  name: your-serviceaccount
roleRef:
  kind: Role
  name: configmap-reader
  apiGroup: rbac.authorization.k8s.io
```

## 常见问题

**Q: 为什么没有读取到配置？**

A: 检查以下几点：
1. 确认 ConfigMap 存在：`kubectl get configmap example-config -n default`
2. 确认 Namespace 配置正确
3. 确认 Key 配置正确
4. 检查 RBAC 权限

**Q: 配置更新后没有收到通知？**

A: 可能的原因：
1. `Watch` 参数设置为 `false`
2. ConfigMap 内容没有变化（相同内容不会触发更新）
3. Kubernetes 集群网络问题
4. Watcher channel 满，丢弃了更新

**Q: 如何处理大量配置？**

A: 建议的方法：
1. 使用多个 ConfigMap，按模块拆分
2. 使用 `MergeAllKey` 模式合并多个 key
3. 考虑使用 Secret 存储敏感配置

**Q: ConfigMap 大小限制是多少？**

A: ConfigMap 的大小限制为 1 MiB（1,048,576 字节）。如果配置过大，建议：
1. 拆分为多个 ConfigMap
2. 使用外部配置中心（如 etcd）
3. 将大文件存储在 ConfigMap 中引用的 Volume 中

**Q: 如何使用不同的 kubeconfig 文件？**

A: 设置 `Kubeconfig` 参数：
```go
src, err := k8s.NewConfigMapSource(k8s.ConfigSourceConfig{
    Kubeconfig: "/path/to/kubeconfig.yaml",
    // ...
})
```

**Q: 支持配置加密吗？**

A: ConfigMap 本身不加密，如果需要加密：
1. 使用 Secret 存储敏感信息（但 Secret 只是 Base64 编码）
2. 使用 Kubernetes 的加密功能（如 KMS 加密）
3. 在应用层解密配置
4. 使用密钥管理服务（如 HashiCorp Vault）

## 最佳实践

1. **使用环境隔离**：不同环境使用不同的 ConfigMap（如 `app-config-dev`、`app-config-prod`）
2. **配置分层**：将配置分为基础配置、环境配置、实例配置，按优先级加载
3. **配置验证**：在代码中验证配置的合法性
4. **配置监控**：记录配置变更日志，便于审计和排查问题
5. **配置版本管理**：在配置中包含版本信息，便于回滚
6. **敏感信息**：敏感信息（密码、密钥等）应使用 Secret 存储
7. **配置热更新**：合理配置 `Watch`，实现配置热更新
8. **RBAC 权限**：使用最小权限原则配置 RBAC

## 相关文档

- [k8s 主文档](../../readme.md)
- [Secret 配置源示例](../secret-source/)
- [Resolver 示例](../resolver/)
- [yggdrasil 配置文档](../../../../docs/config.md)

## 退出

按 `Ctrl+C` 退出程序。
