...
import (
	...
	_ "github.com/codesjoy/yggdrasil/contrib/etcd/v2"
	...
	go func() {
		...
		for range time.Tick(5 * time.Second) {
			...
			inst := demoInstance{
				...
				endpoints: []registry.Endpoint{
					demoEndpoint{scheme: "grpc", address: fmt.Sprintf("127.0.0.1:%d", 9000+instanceCount)},
					demoEndpoint{scheme: "http", address: fmt.Sprintf("127.0.0.1:%d", 8080+instanceCount)},
				},
			}
